...
- URL: departamentos alquiler + alquiler-temporal en Capital Federal
- Parseo de precio mejorado: ARS, USD, pesos, "Consultar", precio en detalle
- Columna Tipo_Alquiler: diferencia "Alquiler" de "Alquiler Temporal"
...
SEARCH_URL = f"{BASE_URL}/departamentos/alquiler-o-alquiler-temporal/capital-federal"
...
def inferir_tipo_alquiler(url: str, texto_card: str = "") -> str:
    """
    Determina si es alquiler temporal o alquiler tradicional.
    Prioridad: texto del card → URL de la publicación.
    """
    texto_lower = texto_card.lower()
    url_lower   = url.lower()

    if "temporal" in texto_lower or "alquiler-temporal" in url_lower:
        return "Alquiler Temporal"
    if "alquiler" in url_lower or "alquiler" in texto_lower:
        return "Alquiler"
    return "N/A"
...
def parse_listing_page(html: str) -> tuple[list[dict], bool]:
    ...
    for item in items_raw:
        try:
            ...
            # ── Tipo de alquiler ───────────────────────────────────────────
            # Primero buscamos un badge/tag explícito en el card
            tipo_tag = (
                item.find("span", class_=re.compile(r"tag|badge|tipo|operacion", re.I))
                or item.find("p",    class_=re.compile(r"tag|badge|tipo|operacion", re.I))
            )
            tipo_texto_card = clean_text(tipo_tag.text) if tipo_tag else ""

            # Si no hay badge, usamos el texto completo del card + la URL como señal
            if not tipo_texto_card or tipo_texto_card == "N/A":
                tipo_texto_card = clean_text(item.get_text())

            tipo_alquiler = inferir_tipo_alquiler(link, tipo_texto_card)
...
def parse_detail_page(html: str, url: str = "") -> dict:
    ...
    # ── Tipo de alquiler desde detalle (refuerzo) ─────────────────────────────
    # Argenprop suele mostrar "Alquiler temporal" en un <h2> o breadcrumb
    tipo_desde_detalle = ""
    for sel in ["h1", "h2", ".breadcrumb", "[class*='operacion']", "[class*='tipo']"]:
        el = soup.select_one(sel)
        if el:
            txt = clean_text(el.get_text()).lower()
            if "temporal" in txt:
                tipo_desde_detalle = "Alquiler Temporal"
                break
            if "alquiler" in txt:
                tipo_desde_detalle = "Alquiler"
    result["Tipo_Alquiler_Detalle"] = tipo_desde_detalle or inferir_tipo_alquiler(url)
...
def merge_item(item: dict) -> dict:
    # Precio: si el listado dio "Consultar", usar el del detalle
    if item.get("Precio") == "Consultar":
        ficha = item.get("Precio_Ficha", "N/A")
        if ficha and ficha not in ("N/A", "Consultar"):
            item["Precio"] = ficha

    # Tipo_Alquiler: si el listado no lo detectó bien, usar el del detalle
    if item.get("Tipo_Alquiler") == "N/A":
        detalle = item.get("Tipo_Alquiler_Detalle", "N/A")
        if detalle and detalle != "N/A":
            item["Tipo_Alquiler"] = detalle
...
def save_incremental(data: list[dict], filepath: str):
    ...
    df.drop(columns=["Tipo_Alquiler_Detalle"], errors="ignore", inplace=True)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Argenprop Scraper")
